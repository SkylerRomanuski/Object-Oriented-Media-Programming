...
final int strokeSize = 3 ; // basis for strokeWeight()
...
PImage STUDENTIMAGE ;
PImage BACKGROUND;
PShape STUDENTSHAPE, STUMP, BRANCH, STAR ; // STUDENT I set this via createShape in setup() but use this in drawShape().
...
void setup() {
  ...
  //image from https://thecareyadventures.com/blog/photo-of-the-day-ponderosa-pine-bark-texture/
  STUDENTIMAGE = loadImage("pine.jpg");
  ...
  push();
  STUDENTSHAPE = createShape(GROUP);
  beginShape();
  strokeWeight(0);
  fill(color(39, 96, 46));
  STUMP = createShape(RECT, 25, 25, 18, 100); //make a brown rectangle
  STUMP.setTexture(STUDENTIMAGE); //set the stump texture to the rectangle
  fill(color(120, 96, 46));
  BRANCH = createShape(TRIANGLE, 0, -25, 50, -25, 25, -150); //make a green triangle
  fill(color(50.6, 100, 100));
  STAR = createShape();
  STAR.beginShape(); //create a yellow star
  STAR.vertex(25, -150); 
  STAR.vertex(28, -140); 
  STAR.vertex(34, -140);
  STAR.vertex(30, -132);
  ...
  STAR.vertex(25, -150); 
  STAR.endShape(CLOSE);
  STUDENTSHAPE.addChild(STUMP);
  STUDENTSHAPE.addChild(BRANCH);
  STUDENTSHAPE.addChild(STAR);
  endShape(CLOSE);
  pop();
}
...
void drawRecursiveShape(final int mydepth, final int recursionDepth, final float rotation) {
  if (mydepth >= recursionDepth) {
    ...
    helpRotate();
    drawShape(mydepth, rotation); //keeps a copy of the shape in the center
  }
}
...
void drawShape(int mydepth, final float rotation) {
  ...
  translate(0, 500);
  push();
  for(mydepth = 0; mydepth <= recursionDepth; mydepth++){
    scale(1.0/(mydepth + 1));
    translate(0,-250);
    shape(STUDENTSHAPE, 0, 0, 500, 500); //draw the shape in Z direction
    push();
    rotateY(radians(90)); 
    shape(STUDENTSHAPE, 0, 0, 500, 500); //draw the shape in Y direction
    pop();
    push();
    translate(0, -85, 85);
    rotateX(radians(90)); 
    shape(STUDENTSHAPE, 0, 0, 500, 500); //draw the shape in X direction
    pop();
  }
  pop();
...
void keyPressed() {
  ...
  } else if(key == '2'){ //added functions to the '2', '4', '6', and '8' keys
    spanSteps = 2;
  } else if(key == '4'){
    spanSteps = 4;
  } else if(key == '6'){
  ...
  if (keyPressed) {
    if (! (controlKey || altKey)) {
      ...
      if (controlKey) {
        ...
        if (key == 'x' || ((int(key) == 24 || (key == CODED && keyCode == 88)) && ! shiftKey)) { //CONTROL-x, CONTROL-y, and CONTROL-z: continuously incremented by spanIncr.
        ...
        } else if (key == 'X'|| ((int(key) == 24 || (key == CODED && keyCode == 88)) && shiftKey)) { //CONTROL-X, CONTROL-Y, and CONTROL-Z: continuously decremented by spanIncr
          xSpanLimit = constrain(xSpanLimit - spanIncr, -xSpanRange, xSpanRange);
        } else if (key == 'Y'|| ((int(key) == 25 || (key == CODED && keyCode == 89)) && shiftKey)) {
          ySpanLimit = constrain(ySpanLimit - spanIncr, -ySpanRange, ySpanRange);
        } else if (key == 'Z'|| ((int(key) == 26 || (key == CODED && keyCode == 90)) && shiftKey)) {
      ...
      if (altKey) {
        if (key == 'x') { //ALT-x, ALT-y, and ALT-z: continuously incremented by spanIncr.
          ...
          zScaleAmount = constrain(zScaleAmount + spanIncr, -zSpanRange, zSpanRange); //ALT-X, ALT-Y, and ALT-Z: continuously decremented by spanIncr.
        } else if (key == 'X') {
          xScaleAmount = constrain(xScaleAmount - spanIncr, -xSpanRange, xSpanRange);
        } else if (key == 'Y') {
          yScaleAmount = constrain(yScaleAmount - spanIncr, -ySpanRange, ySpanRange);
